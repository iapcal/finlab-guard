...
class TestCacheManagerDeletions:
    ...
    def test_detect_row_deletions(self, cache_manager):
        ...
        cell_df, row_df, row_del_df, col_add_df, col_del_df, meta = cache_manager._get_changes_extended_polars(
            base_df, modified_df, timestamp
        )
    ...
    def test_mixed_operations(self, cache_manager):
        ...
        }, index=["row1", "row3", "row4"])  # row4 added
    ...
    def test_save_deletions(self, cache_manager):
        ...
        modified_df = pd.DataFrame({
            "A": [1, 3]
        }, index=["row1", "row3"])  # row2 deleted, column B deleted
    ...
    def test_reconstruct_with_deletions(self, cache_manager):
        ...
        result = cache_manager.reconstruct_as_of("test_table", timestamp2 + timedelta(seconds=1))
    ...
    def test_time_based_deletion_filtering(self, cache_manager):
        ...
        result_before = cache_manager.reconstruct_as_of("test_table", timestamp1 + timedelta(minutes=1))
        assert len(result_before) == 3
        assert "row2" in result_before.index

        # Reconstruct after deletion (should not include row2)
        result_after = cache_manager.reconstruct_as_of("test_table", timestamp2 + timedelta(minutes=1))
    ...
    def test_load_deletions_method(self, cache_manager):
        ...
        # Load deletions
        deleted_rows, deleted_cols = cache_manager._load_deletions("test_table", timestamp + timedelta(minutes=1))
    ...
    def test_delete_then_readd(self, cache_manager):
        ...
        result1 = cache_manager.reconstruct_as_of("test_table", timestamp1 + timedelta(seconds=30))
        assert len(result1) == 3
        assert "row2" in result1.index

        result2 = cache_manager.reconstruct_as_of("test_table", timestamp2 + timedelta(seconds=30))
        assert len(result2) == 2
        assert "row2" not in result2.index

        result3 = cache_manager.reconstruct_as_of("test_table", timestamp3 + timedelta(seconds=30))
    ...
    def test_progressive_deletions(self, cache_manager):
        ...
        result4 = cache_manager.reconstruct_as_of("test_table", timestamp4 + timedelta(seconds=30))
    ...
    def test_deletions_with_special_values(self, cache_manager):
        ...
        modified_df = pd.DataFrame({
            ...
            "B": ["x", "z"],
            "C": [10, -np.inf]
        }, index=["row1", "row3"])  # row2 (inf) and row4 (nan) deleted
        ...
        result = cache_manager.reconstruct_as_of("test_special", timestamp2 + timedelta(seconds=1))
    ...
    def test_delete_columns_with_special_values(self, cache_manager):
        ...
        timestamp2 = timestamp1 + timedelta(minutes=1)
        cache_manager.save_version("test_special_cols", base_df, modified_df, timestamp2)

        result = cache_manager.reconstruct_as_of("test_special_cols", timestamp2 + timedelta(seconds=1))
    ...
    def test_json_serialization_of_special_values(self, cache_manager):
        ...
        timestamp2 = timestamp1 + timedelta(minutes=1)
        cache_manager.save_version("test_json_special", base_df, modified_df, timestamp2)

        # Check that deletion records were created (they should be stored as strings)
        row_del_query = "SELECT * FROM row_deletions WHERE table_id = 'test_json_special'"
        row_del_result = cache_manager.conn.execute(row_del_query).fetchdf()
        ...
        result = cache_manager.reconstruct_as_of("test_json_special", timestamp2 + timedelta(seconds=1))
        assert len(result) == 1
    ...
    def test_complex_inf_nan_deletion_readd_cycles(self, cache_manager):
        ...
        }, index=["A", "E", "F"])  # Different index for inf value
        ...
        }, index=["A", "E", "D"])  # Original nan row index
        ...
        step5_df = pd.DataFrame({
            ...
            "status": ["normal", "normal2", "restored_nan", "new_inf2", "new_neg_inf", "zero"]
        }, index=["A", "E", "D", "G", "H", "I"])
        ...
        # Test reconstruction at each step
        result1 = cache_manager.reconstruct_as_of("complex_test", timestamp1 + timedelta(seconds=30))
        ...
        result2 = cache_manager.reconstruct_as_of("complex_test", timestamp2 + timedelta(seconds=30))
        assert len(result2) == 2
        assert "B" not in result2.index and "C" not in result2.index and "D" not in result2.index

        result3 = cache_manager.reconstruct_as_of("complex_test", timestamp3 + timedelta(seconds=30))
        assert len(result3) == 3
        ...
        result4 = cache_manager.reconstruct_as_of("complex_test", timestamp4 + timedelta(seconds=30))
        ...
        result5 = cache_manager.reconstruct_as_of("complex_test", timestamp5 + timedelta(seconds=30))
    ...
    def test_inf_nan_column_operations(self, cache_manager):
        ...
        step5_df = pd.DataFrame({
            "normal": [1, 2, 3],
            "new_special": [-np.inf, 0.0, np.inf],
            "pure_inf": [-np.inf, -np.inf, -np.inf]  # Different values than original
        }, index=["row1", "row2", "row3"])

        timestamp5 = timestamp4 + timedelta(minutes=1)

        # Debug: Check what changes are detected for step 5
        cell_df5, row_df5, row_del_df5, col_add_df5, col_del_df5, meta5 = cache_manager._get_changes_extended_polars(
            step4_df, step5_df, timestamp5
        )
        ...
        result1 = cache_manager.reconstruct_as_of("col_test", timestamp1 + timedelta(seconds=30))
        assert len(result1.columns) == 4
        assert all(np.isinf(result1["pure_inf"]) & (result1["pure_inf"] > 0))
        assert all(np.isnan(result1["pure_nan"]))

        result2 = cache_manager.reconstruct_as_of("col_test", timestamp2 + timedelta(seconds=30))
        ...
        result3 = cache_manager.reconstruct_as_of("col_test", timestamp3 + timedelta(seconds=30))
        ...
        result4 = cache_manager.reconstruct_as_of("col_test", timestamp4 + timedelta(seconds=30))
        ...
        additions5 = cache_manager._load_and_process_row_additions("col_test", target_time)
        deleted_rows5, deleted_cols5 = cache_manager._load_deletions("col_test", target_time)
        ...
        result5 = cache_manager.reconstruct_as_of("col_test", timestamp5 + timedelta(seconds=30))
    ...
    def test_special_values_edge_cases_with_deletions(self, cache_manager):
        ...
        special_only_df = pd.DataFrame({
            "col1": [np.inf, -np.inf, np.nan],
            "col2": [np.nan, np.inf, -np.inf]
        ...
        # Delete all rows
        empty_df = pd.DataFrame({"col1": [], "col2": []}).astype({"col1": float, "col2": float})
        timestamp2 = timestamp1 + timedelta(minutes=1)
        ...
        }, index=["inf", "neg_inf", "nan"])  # Same indices
        ...
        new_cols_df = pd.DataFrame({
            "new_col1": [np.inf, np.inf, np.inf],
            "new_col2": [1.0, 2.0, 3.0]
        }, index=["inf", "neg_inf", "nan"])
        ...
        result1 = cache_manager.reconstruct_as_of("edge_test", timestamp1 + timedelta(seconds=30))
        assert len(result1) == 3
        ...
        result2 = cache_manager.reconstruct_as_of("edge_test", timestamp2 + timedelta(seconds=30))
        assert len(result2) == 0  # All deleted

        result3 = cache_manager.reconstruct_as_of("edge_test", timestamp3 + timedelta(seconds=30))
        assert len(result3) == 3
        # Swapped pattern
        assert np.isnan(result3.loc["inf", "col1"])
        assert np.isinf(result3.loc["inf", "col2"]) and result3.loc["inf", "col2"] < 0

        result4 = cache_manager.reconstruct_as_of("edge_test", timestamp4 + timedelta(seconds=30))
        assert len(result4) == 3
        assert len(result4.columns) == 2
    ...
    def test_multiple_deletion_addition_cycles(self, cache_manager):
        ...
        result_t1_5 = cache_manager.reconstruct_as_of("cycle_test", timestamp_t1 + timedelta(seconds=30))
        print(f"t1.5 - columns: {list(result_t1_5.columns)}")
        assert "A" in result_t1_5.columns
        assert result_t1_5["A"].tolist() == [1, 2, 3]

        result_t2_5 = cache_manager.reconstruct_as_of("cycle_test", timestamp_t2 + timedelta(seconds=30))
        print(f"t2.5 - columns: {list(result_t2_5.columns)}")
        assert "A" not in result_t2_5.columns  # Column A should be deleted

        result_t3_5 = cache_manager.reconstruct_as_of("cycle_test", timestamp_t3 + timedelta(seconds=30))
        ...
        if "A" in result_t3_5.columns:
            ...
            assert False, "Column A should be restored at t3.5"

        result_t4_5 = cache_manager.reconstruct_as_of("cycle_test", timestamp_t4 + timedelta(seconds=30))
        print(f"t4.5 - columns: {list(result_t4_5.columns)}")
        assert "A" not in result_t4_5.columns  # Column A should be deleted again

        result_t5_5 = cache_manager.reconstruct_as_of("cycle_test", timestamp_t5 + timedelta(seconds=30))
        ...
        if "A" in result_t5_5.columns:
            ...
            assert False, "Column A should be restored again at t5.5"
    ...
    def test_nan_inf_json_round_trip(self, cache_manager):
        ...
        for i in range(3):
            # Delete some rows
            remaining_rows = ["normal", "zero"] if i % 2 == 0 else ["pos_inf", "nan_val"]
            subset_df = original_df.loc[remaining_rows].copy()

            timestamp_del = timestamp1 + timedelta(minutes=i*2+1)
            cache_manager.save_version("json_test", original_df, subset_df, timestamp_del)

            # Re-add all rows
            timestamp_add = timestamp_del + timedelta(minutes=1)
            cache_manager.save_version("json_test", subset_df, original_df, timestamp_add)

        # Final reconstruction should match original
        final_result = cache_manager.reconstruct_as_of("json_test", timestamp_add + timedelta(seconds=30))

        assert len(final_result) == 5
        assert np.isinf(final_result.loc["pos_inf", "float_vals"]) and final_result.loc["pos_inf", "float_vals"] > 0
        assert np.isinf(final_result.loc["neg_inf", "float_vals"]) and final_result.loc["neg_inf", "float_vals"] < 0
    ...
    def test_multiple_row_deletion_addition_cycles(self, cache_manager):
        ...
        for cycle in range(1, 9):
            ...
            cache_manager.save_version("row_cycle_test", current_df, delete_df, delete_timestamp)
            ...
            cache_manager.save_version("row_cycle_test", delete_df, add_df, add_timestamp)

            current_df = add_df

        # Test reconstruction at various points
        # Original state
        result_original = cache_manager.reconstruct_as_of("row_cycle_test", timestamp_t1 + timedelta(seconds=30))
        ...
        for cycle in range(1, 9):
            ...
            # Test deletion phase
            result_delete = cache_manager.reconstruct_as_of("row_cycle_test", delete_time + timedelta(seconds=30))
            assert "row2" not in result_delete.index, f"Cycle {cycle}: row2 should be deleted"
            assert len(result_delete) == 2, f"Cycle {cycle}: should have 2 rows after deletion"

            # Test addition phase
            result_add = cache_manager.reconstruct_as_of("row_cycle_test", add_time + timedelta(seconds=30))
            assert "row2" in result_add.index, f"Cycle {cycle}: row2 should be restored"
            assert result_add.loc["row2", "A"] == cycle * 100, f"Cycle {cycle}: wrong A value"
            assert result_add.loc["row2", "B"] == f"cycle_{cycle}", f"Cycle {cycle}: wrong B value"
            assert len(result_add) == 3, f"Cycle {cycle}: should have 3 rows after addition"

        # Test final state (after all 8 cycles)
        final_timestamp = timestamp_t1 + timedelta(minutes=16)  # 8*2
        final_result = cache_manager.reconstruct_as_of("row_cycle_test", final_timestamp + timedelta(seconds=30))
        ...
        result_cycle_3 = cache_manager.reconstruct_as_of("row_cycle_test", cycle_3_time + timedelta(seconds=30))
        ...
        result_cycle_6 = cache_manager.reconstruct_as_of("row_cycle_test", cycle_6_time + timedelta(seconds=30))
        assert "row2" in result_cycle_6.index
        assert result_cycle_6.loc["row2", "A"] == 600
        assert result_cycle_6.loc["row2", "B"] == "cycle_6"

        print(f"Successfully completed 8 deletion-addition cycles for row2")